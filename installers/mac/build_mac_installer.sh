...
script_file=NeuralSIMDRuntimePlugin.pkgproj
...
pkg_dir=NeuralSIMDRuntimePlugin_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/NeuralSIMDRuntimePlugin.pkg $pkg_dir/NeuralSIMDRuntimePlugin-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/NeuralSIMDRuntimePlugin-signed.pkg --bundle-id com.chowdsp.NeuralSIMDRuntimePlugin --asc-provider "$TEAM_ID" --username chowdsp@gmail.com --password "$INSTALLER_PASS"

echo "Building disk image..."
vol_name=Install_NeuralSIMDRuntimePlugin-$app_version
rm -f "$vol_name"
